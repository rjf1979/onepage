...
1. **落地页** — Header / Hero（文案 + 简历纸视觉 + ATS 徽章浮层）/ 信任条 / 三大价值卡 / 三步流程 / 模板墙（路线图 4 套，已上线 3 套）/ 深色 CTA / Footer
2. **编辑器** — 顶栏（自动保存状态 / 换模板 / ATS 自检 / 导出 PDF）+ 左表单填空 + 右 A4 实时预览 + 量化提醒（本地正则检测，非 AI）
3. **ATS 自检报告** — 兼容分 + 逐项检查（通过/建议）+ 右侧解析高亮预览，明确指出机器能读哪些区块

> 文案纪律：页面上写的每一项能力都必须真的存在。已上线的模板是「经典单栏 / 应届紧凑版 / 学术 CV」，
...
| 模板 | 状态 | 版式 |
|---|---|---|
| 经典单栏 | 已上线 | 单栏，ATS 解析风险低 |
| 应届紧凑版 | 已上线 | 单栏 + 贴边深色抬头条，教育背景前置，行距更密 |
| 学术 CV | 已上线 | 单栏居中抬头，教育背景前置，**允许多页**，导出自动分页 |
| 作品集双栏 | 打磨中 | 视觉分栏（CSS 栅格），DOM 顺序仍为单栏线性，自检标为**中等风险** |

- `templateId` 存在简历数据里，跟着 JSON 备份走；本地存档与导入备份里的非法/未上线 id
  一律回退到经典单栏（见 `resolveTemplateId`），不会渲染成空白。
- 版式组件在 `src/components/resume/`，`ResumePaper.vue` 只做 A4 外壳与分发，
  编辑器预览 / 打印导出 / 落地页 Hero 三处共用同一份渲染。
- ATS 的「排版解析风险」由模板的 `layout` 决定，不再恒为「低」；学术模板会多出一项
  「该模板按多页排版，导出时自动分页」。
- 多页分页靠 `@page { margin }` 提供逐页页边距，条目级 `break-inside: avoid`
  防止某段经历被拦腰截断。
...
- **多套模板**：3 套已上线（经典单栏 / 应届紧凑版 / 学术 CV），换模板即时切换并随数据持久化。
- **ATS 自检**：12 项机器解析检查，给出兼容性分数与可落地建议；排版风险按模板如实标注。
...
- [x] **M2 模板系统**：`templateId` 进数据模型与持久化，版式组件拆分，换模板真正可切换
- [ ] **M2.5 作品集双栏**：按已定方案实现 DOM 顺序安全的视觉分栏，自检标为中等风险

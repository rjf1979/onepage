...
1. **落地页** — Header / Hero（文案 + 简历纸视觉 + ATS 徽章浮层）/ 信任条 / 三大价值卡 / 三步流程 / 模板墙（4 套全部上线）/ 深色 CTA / Footer
2. **编辑器** — 顶栏（自动保存状态 / 换模板 / ATS 自检 / 导出 PDF）+ 左表单填空 + 右 A4 实时预览 + 量化提醒（本地正则检测，非 AI）
3. **ATS 自检报告** — 兼容分 + 逐项检查（通过/建议）+ 右侧解析高亮预览，明确指出机器能读哪些区块

> 文案纪律：页面上写的每一项能力都必须真的存在。已上线的模板是
> 「经典单栏 / 应届紧凑版 / 学术 CV / 作品集双栏」，
...
| 作品集双栏 | 已上线 | 视觉分栏（CSS 栅格），DOM 顺序仍为单栏线性，自检标为**中等风险** |

### 双栏为什么能同时叫「ATS 友好」和「中等风险」

分栏只用 CSS 栅格摆放两个普通流式区块，**不用表格 / 浮动 / 绝对定位**；
且在 DOM 里正文（项目 + 经历）在前、侧栏（技能 + 教育）在后，靠 `order` 把侧栏摆到视觉左侧。
机器按文档顺序取文本时，拿到的是一段完整连贯的简历，而不是被栅格切碎、按行交替的碎片。

代价要说清楚：少数按「坐标」而非「文档顺序」取词的解析器仍可能打乱顺序。
所以自检把它标为**中等风险**（评分相应扣 7 分），落地页与换模板弹层上也都挂了
「ATS 中风险」标签，不靠用户自己发现。
...
- ATS 的「排版解析风险」由模板的 `layout` 决定，不再恒为「低」；学术模板会多出一项
  「该模板按多页排版，导出时自动分页」，双栏模板的解析示意脚注也会跟着改措辞。
- 多页分页靠 `@page { margin }` 提供逐页页边距，条目级 `break-inside: avoid`
  防止某段经历被拦腰截断。
- 分栏与深色抬头条的底色都用 `print-color-adjust: exact` 保色；实测导出的 PDF
  内容流里确实含 `#FAF7F0`（侧栏底）与 `#F0E9DB`（技能胶囊）。
...
- **多套模板**：4 套全部上线（经典单栏 / 应届紧凑版 / 学术 CV / 作品集双栏），换模板即时切换并随数据持久化。
...
- [x] **M2.5 作品集双栏**：DOM 顺序安全的视觉分栏已上线，自检如实标为中等风险并挂出标签
